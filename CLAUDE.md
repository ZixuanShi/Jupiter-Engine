...
`libJupiterEngine.a`; the executable comes from the active project under `Projects/<Name>/`. Both
land in that project's own `_Output/<preset>/`, beside its `_ProjectFiles/` and `_Saved/`. One project
per build tree — the root `CMakeLists.txt` ends with `add_subdirectory(${JUPITER_PROJECT})`, and
`setup.py` chooses it.

**The archive is per project, not shared, and must stay that way.** It used to build to
`${CMAKE_SOURCE_DIR}/_Output/`, which two projects then wrote to in turn while its *contents* stayed
project-specific — `JUPITER_PROJECT_DIR` is compiled into it. Ninja in one project's tree would find
that archive newer than its own objects, skip the `ar` step, and link the other project's engine.
Measured on 2026-09-14: a `UnitTests` run whose `GetSavedDir()` named `Projects/Blank/_Saved`, with
`strings` on the archive confirming Blank's path was the only one in it. Nothing warns.
...
Final binary and the engine archive both land in `Projects/<Name>/_Output/<preset>/`; nothing is
written to a repo-root `_Output/` any more. Both `_ProjectFiles/` and `_Output/` are gitignored.
