...
bool UnitTests_Clock()
{
    ...
    JPT_INFO("50 minutes duration: " + jpt::ToString(duration));
    JPT_INFO("50 minutes later: "    + jpt::ToString((now + duration)));
    JPT_INFO("50 minutes before: "   + jpt::ToString((now - duration)));
...
{    
    ...
    JPT_INFO("10 years ago: " + jpt::ToString(now));

    // Log the time 20 years later
    now.Forward(60 * 60 * 24 * 365 * 20);
    JPT_INFO("10 years later: " + jpt::ToString(now));
...
{
    //JPT_INFO("Current Time: " + jpt::Clock::GetCurrentDateTime().ToString());
