...
namespace jpt::local
{
    ...
    void ScreenDeltaTest(TestCase& test)
    {
        Camera camera;
        ...
        test.Expect(AreValuesClose(right, Vec3(0.57735f, 0.0f, 0.0f), 1e-4f), "A rightward drag mapped to ({}, {}, {})", right.x, right.y, right.z);

        // Screen Y is down and world Y is up, so the sign flips.
        const Vec3 down = camera.ScreenDeltaToWorld(Vec2(0.0f, 100.0f), 1000.0f);
        test.Expect(AreValuesClose(down, Vec3(0.0f, -0.57735f, 0.0f), 1e-4f), "A downward drag mapped to ({}, {}, {})", down.x, down.y, down.z);
        ...
        test.Expect(AreValuesClose(farther.x, right.x * 2.0f, 1e-4f), "Doubling the distance scaled the drag by {}", farther.x / right.x);
        ...
        test.Expect(AreValuesClose(pivotBefore, pivotAfter, 1e-4f), "Zoom dragged the orbit point from ({}, {}, {}) to ({}, {}, {})", pivotBefore.x, pivotBefore.y, pivotBefore.z, pivotAfter.x, pivotAfter.y, pivotAfter.z);
        ...
        test.Expect(AreValuesClose(halved.x, right.x, 1e-4f), "Zoom(0.5) from 10 units did not land at 5");
        camera.Zoom(2.0f);
        const Vec3 restored = camera.ScreenDeltaToWorld(Vec2(100.0f, 0.0f), 1000.0f);
        test.Expect(AreValuesClose(restored.x, farther.x, 1e-4f), "Zoom(2) did not undo Zoom(0.5)");
    ...
    void BasisTest(TestCase& test)
    {
        ...
        test.Expect(AreValuesClose(right.Length(), 1.0f, 1e-4f), "Camera right is {} long", right.Length());
        test.Expect(AreValuesClose(up.Length(), 1.0f, 1e-4f), "Camera up is {} long", up.Length());
        test.Expect(AreValuesClose(right.Dot(up), 0.0f, 1e-4f), "Camera right and up are {} apart", right.Dot(up));

        // The premise of the assertion below, so it cannot pass by being vacuous.
        test.Expect(!AreValuesClose(right.Dot(Vec3::Right()), 1.0f, 1e-3f), "This camera is on-axis, so it cannot tell world X from screen right");
        ...
        test.Expect(AreValuesClose(tipped.Dot(right), 0.0f, 1e-4f), "A vertical drag slid the object {} sideways on screen", tipped.Dot(right));
        ...
        for (int32 i = 0; i < 6; ++i)
        {
            ...
            test.Expect(AreValuesClose(rolled, 0.0f, 1e-4f), "The model rolled {} sideways after {} drags", rolled, i + 1);
        }
        ...
        test.Expect(!AreValuesClose(forward.Dot(Vec3::Forward()), 1.0f, 1e-3f), "This camera looks down -Z, so it cannot tell world Z from the view axis");
        ...
        test.Expect(AreValuesClose(depthAfter, depthBefore, 1e-4f), "A twist moved the model {} along the view axis", depthAfter - depthBefore);
    }

    /** Local move and rotate, which the Dev Menu drives. */
    void LocalMotionTest(TestCase& test)
    {
        Camera camera;
        ...
        test.Expect(AreValuesClose(camera.Forward(), Vec3(-1.0f, 0.0f, 0.0f), 1e-4f), "Yaw 90 aimed at ({}, {}, {})", camera.Forward().x, camera.Forward().y, camera.Forward().z);

        // Move is along the camera's axes, not the world's: +x is its right, which is now -Z.
        camera.MoveLocal(Vec3(2.0f, 0.0f, 0.0f));
        test.Expect(AreValuesClose(camera.GetPosition(), Vec3(0.0f, 0.0f, -2.0f), 1e-4f), "Local +x moved to ({}, {}, {}), expected (0, 0, -2)", camera.GetPosition().x, camera.GetPosition().y, camera.GetPosition().z);
        ...
        for (int32 i = 0; i < 8; ++i)
        {
            ...
            test.Expect(AreValuesClose(camera.Right().y, 0.0f, 1e-4f), "The camera rolled: right.y is {} after {} rotations", camera.Right().y, i + 1);
        }
        ...
        test.Expect(AreValuesClose(camera.Forward(), Vec3::Forward(), 1e-4f), "LookAt aimed at ({}, {}, {})", camera.Forward().x, camera.Forward().y, camera.Forward().z);
        test.Expect(AreValuesClose(camera.GetDistance(), 8.0f, 1e-4f), "LookAt left the orbit distance at {}, expected 8", camera.GetDistance());
        ...
        test.Expect(AreValuesClose(orbit, Vec3(0.0f, 0.0f, 2.0f), 1e-4f), "The orbit point landed at ({}, {}, {})", orbit.x, orbit.y, orbit.z);
        ...
        test.Expect(AreValuesClose(camera.GetViewProjection(1.5f), viaLookAt, 1e-3f), "The quaternion view matrix disagrees with LookAt");
    }
    ...
    void MoveAxisTest(TestCase& test)
    {
        Input input;
        test.Expect(GetMoveAxis(input) == Vec3::Zero(), "An idle keyboard asked the camera to move");

        input.PostKeyDown(KeyCode::D, false);
        test.Expect(GetMoveAxis(input) == Vec3(1.0f, 0.0f, 0.0f), "D did not map to the camera's right");
        ...
        test.Expect(GetMoveAxis(input) == Vec3(0.0f, 1.0f, 0.0f), "E did not map to the camera's up");
        input.PostKeyUp(KeyCode::E);

        input.PostKeyDown(KeyCode::Q, false);
        test.Expect(GetMoveAxis(input) == Vec3(0.0f, -1.0f, 0.0f), "Q did not map to the camera's down");
        input.PostKeyUp(KeyCode::Q);

        input.PostKeyDown(KeyCode::W, false);
        test.Expect(GetMoveAxis(input) == Vec3(0.0f, 0.0f, -1.0f), "W did not map to the camera's forward");

        // Arrows are the same axis, so a held W and a held UpArrow must not stack.
        input.PostKeyDown(KeyCode::UpArrow, false);
        test.Expect(GetMoveAxis(input) == Vec3(0.0f, 0.0f, -1.0f), "W and UpArrow stacked into a double-speed press");
        ...
        test.Expect(GetMoveAxis(input) == Vec3::Zero(), "W and S did not cancel");
        ...
        test.Expect(AreValuesClose(diagonal.Length(), 1.0f, 1e-4f), "A diagonal press moves at {}, expected 1", diagonal.Length());
        test.Expect(diagonal.x < 0.0f && diagonal.z < 0.0f, "A+W pointed ({}, {}, {}), expected forward and left", diagonal.x, diagonal.y, diagonal.z);
    }

    static TestCase s_screenDelta("Scene.CameraScreenDelta", &ScreenDeltaTest);
    static TestCase s_basis("Scene.CameraBasis", &BasisTest);
    static TestCase s_localMotion("Scene.CameraLocalMotion", &LocalMotionTest);
    static TestCase s_moveAxis("Scene.CameraMoveAxis", &MoveAxisTest);
}
