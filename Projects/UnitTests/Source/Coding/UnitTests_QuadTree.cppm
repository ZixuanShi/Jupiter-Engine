...
private:
    std::vector<jpt::Vec2d> m_objects;
    usize m_probes = 0;

public:
    QuadTreeSearch(std::vector<jpt::Vec2d> objects);

public:
    [[nodiscard]] std::vector<jpt::Vec2d> QueryObjects(float64 x, float64 y, float64 length);
...
private:
    ...
    void RecurQueryObjects(float64 x, float64 y, float64 length, usize depth, std::vector<jpt::Vec2d>& results);
...
QuadTreeSearch::QuadTreeSearch(std::vector<jpt::Vec2d> objects)
    : m_objects(std::move(objects))
{
}

std::vector<jpt::Vec2d> QuadTreeSearch::QueryObjects(float64 x, float64 y, float64 length)
{
    std::vector<jpt::Vec2d> results;
    m_probes = 0;
...
void QuadTreeSearch::RecurQueryObjects(float64 x, float64 y, float64 length, usize depth, std::vector<jpt::Vec2d>& results)
{
    ...
    if (depth >= kMaxDepth)
    {
        results.push_back(jpt::Vec2d(x + length / 2.0, y + length / 2.0));
        return;
    }
...
bool QuadTreeSearch::HasObject(float64 x, float64 y, float64 length)
{
    ...
    return std::ranges::any_of(m_objects, [x, y, length](const jpt::Vec2d& object)
        {
...
export void RunUnitTests_QuadTree(jpt::TestCase& test)
{
    ...
    QuadTreeSearch single({ jpt::Vec2d(1.2, 3.7) });
    test.Expect(single.QueryObjects(0.0, 0.0, 16.0) == std::vector<jpt::Vec2d>{ jpt::Vec2d(1.5, 3.5) }, "one object, reported at its cell centre");
    test.Expect(single.GetProbeCount() == 17, "found in 17 probes, not the 256 leaf cells");

    QuadTreeSearch sameCell({ jpt::Vec2d(1.2, 3.7), jpt::Vec2d(1.9, 3.1) });
    test.Expect(sameCell.QueryObjects(0.0, 0.0, 16.0) == std::vector<jpt::Vec2d>{ jpt::Vec2d(1.5, 3.5) }, "two objects in one cell collapse to one centre");

    // Traversal order, not input order: SW, SE, NW, NE at every level
    QuadTreeSearch corners({ jpt::Vec2d(15.2, 15.9), jpt::Vec2d(0.1, 0.1), jpt::Vec2d(15.5, 0.5), jpt::Vec2d(0.5, 15.5) });
    test.Expect(corners.QueryObjects(0.0, 0.0, 16.0) == std::vector<jpt::Vec2d>{ jpt::Vec2d(0.5, 0.5), jpt::Vec2d(15.5, 0.5), jpt::Vec2d(0.5, 15.5), jpt::Vec2d(15.5, 15.5) }, "results come back SW, SE, NW, NE");

    QuadTreeSearch outside({ jpt::Vec2d(-1.0, 2.0), jpt::Vec2d(20.0, 20.0) });
    ...
    QuadTreeSearch midpoint({ jpt::Vec2d(8.0, 8.0) });
    test.Expect(midpoint.QueryObjects(0.0, 0.0, 16.0) == std::vector<jpt::Vec2d>{ jpt::Vec2d(8.5, 8.5) }, "an object on a shared boundary is reported once");

    QuadTreeSearch farEdge({ jpt::Vec2d(16.0, 4.0) });
    ...
    QuadTreeSearch subRegion({ jpt::Vec2d(10.3, 12.7), jpt::Vec2d(1.0, 1.0) });
    test.Expect(subRegion.QueryObjects(8.0, 8.0, 8.0) == std::vector<jpt::Vec2d>{ jpt::Vec2d(10.25, 12.75) }, "a sub-region queries only itself");
}
