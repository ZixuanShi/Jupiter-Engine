...
namespace jpt
{
	inline bool locIsValid(char c)
	{
		return c && c != ',' && c != ' ' && c != '\t';
	}

	inline bool locIsWhiteSpace(char c)
	{
		return c == ',' || c == ' ' || c == '\t';
	}
	...
	JPT_API static EnumSearchInfo BuildEnumStringInfos(const char* rawEnumString, EnumStringInfo* infos, int32 start, size_t count)
	{
	...
	JPT_API static jpt::string GetSubString(int32 value, const EnumSearchInfo& searchInfo)
	{
	...
	JPT_API static jpt::string_view GetSubStringView(int32 value, const EnumSearchInfo& searchInfo)
	{
		value -= searchInfo.m_startValue;
		JPT_ASSERT(value >= 0 && value < searchInfo.m_count, "Invalid value to search for string_view");
