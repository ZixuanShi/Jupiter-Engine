...
namespace jpt
{
    ...
    namespace
    {
        ...
        /** NSEventModifierFlagShift says only that *some* shift is down, so the low bits here are
            the hardware-dependent ones that name a side. Same table imgui_impl_osx uses.

            fn is absent: NSEventModifierFlagFunction also goes high for arrows and F1-F20. */
