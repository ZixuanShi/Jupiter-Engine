...
export namespace jpt
{
	...
	void Renderer_Vulkan::CreateImage(uint32 width, uint32 height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory)
	{
		...
		if (vkCreateImage(m_logicalDevice.GetHandle(), &imageInfo, nullptr, &image) != VK_SUCCESS)
		{
			JPT_ERROR("Failed to create image");
		}

		VkMemoryRequirements memRequirements;
		vkGetImageMemoryRequirements(m_logicalDevice.GetHandle(), image, &memRequirements);
		...
		if (vkAllocateMemory(m_logicalDevice.GetHandle(), &allocInfo, nullptr, &imageMemory) != VK_SUCCESS)
		{
	...
	void Renderer_Vulkan::TransitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout)
	{
		...
		EndSingleTimeCommand(m_logicalDevice, m_memoryTransferCommandPool, commandBuffer);
	}
